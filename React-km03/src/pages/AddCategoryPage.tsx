...
const HomePage = () => {
    ...
    const { data: createdCategory, loading, postData } = usePost<Category>(`${apiUrl}/category/create`); // Använd data för den skapade kategorin

    const [formData, setFormData] = useState<Category>({ name: '', description: '' }); // Separat state för formulärdata
    const [errorForm, setErrorForm] = useState<ErrorForm>({ name: '', description: '' }); // State för felmeddelanden från formuläret
        ...
        <>
            <div className="form-container">
                <form onSubmit={submitForm}>
                    {errorForm.name && <p className="error">{errorForm.name}</p>} {/* Visa felmeddelande för namn */}
                    ...
                    {errorForm.description && <p className="error">{errorForm.description}</p> } {/*  Visa felmeddelande för beskrivning  */}
